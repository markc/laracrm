...
- Pest 4.3.1 for testing, Laravel Pint 1.27.0 for formatting
- SQLite (dev), spatie/laravel-permission for RBAC
- Laravel MCP 0.5.2 for AI tooling integration

### Package Versions (from Boost roster)
...
app/
├── Models/
│   ├── Accounting/    # Account, Invoice, InvoiceItem, Payment, PaymentAllocation,
│   │                  # Quote, QuoteItem, Product, JournalEntry, JournalEntryLine,
│   │                  # BankAccount, Expense, TaxRate
...
31 tables total. Key domain tables and relationships:

**CRM Domain:**
- `customers` - customer_number (unique), type (individual/company), billing/shipping_address (JSON)
- `contacts` - belongsTo Customer, is_primary flag
- `opportunities` - belongsTo Customer, stage tracking, probability, expected_close_date
- `activities` - polymorphic (customer_id, contact_id, opportunity_id), activity_date, due_date

**Accounting Domain:**
- `accounts` - hierarchical chart (parent_id self-ref), type, normal_balance, code (unique)
- `invoices` - belongsTo Customer/Quote, invoice_number (unique), status tracking, balance_due
- `invoice_items` - belongsTo Invoice/Product, quantity, pricing, tax calculations
- `quotes` - belongsTo Customer/Opportunity, quote_number (unique), valid_until, converts to Invoice
- `quote_items` - belongsTo Quote/Product, mirrors InvoiceItem structure
- `payments` - belongsTo Customer/BankAccount, allocated_amount vs unallocated_amount
- `payment_allocations` - links Payments to Invoices (many-to-many with amount)
- `products` - sku (unique), income_account_id, expense_account_id for GL integration
- `journal_entries` - entry_number (unique), is_posted, polymorphic reference_type/id
- `journal_entry_lines` - belongsTo JournalEntry/Account, debit_amount, credit_amount
- `bank_accounts` - belongsTo Account (GL), bank_name, account_number, bsb
- `expenses` - belongsTo Vendor(Customer)/Account/BankAccount, is_billable, customer_id
- `tax_rates` - code (unique), rate, belongsTo Account (tax liability)
